...
from gslice.utils.variables import get_lags_for_freq
...
class TSFlowPS(TSFlowBase):
    def __init__(
        ...
        if use_lags:
            lag_span = max(int(context_length) * 7, int(self.prior_context_length) - int(context_length))
            self.lags_seq = get_lags_for_freq(
                frequency,
                context_length=int(context_length),
                max_lag=max(int(context_length), int(lag_span)),
            )
